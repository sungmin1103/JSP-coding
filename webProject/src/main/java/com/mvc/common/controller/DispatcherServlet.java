...
public class DispatcherServlet extends HttpServlet {
  ...
  public void init() throws ServletException {
	...
	private void process(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// 2. HandlerMapping를 통해 path에 해당하는 Conrtoller를 검색한다.
		//				handerMapping.getController("/board/getBoardList.do")
		Controller ctrl = handlerMapping.getController(path);
		
		// 3. 검색된 Controller를 실행한다.
		// GetBoardListController 클래스의 execute() 메서드 호출
		String viewName = ctrl.execute(request, response);
		
		// 4. ViewResolver를 통해 viewName에 해당하는 화면을 검객한다.
		String view = null;
